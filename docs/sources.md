...
| **X API** — Personalized trends `GET /2/users/personalized_trends` | `trend_name`, `post_count`, `category`, `trending_since` — the only X endpoint with a volume | **App-only is forbidden (verified 403)**; needs OAuth 1.0a or 2.0 **user context** | The counts in X's documentation example (`#AI`, `tweet_count: 125000`) are illustrative: the real field is `post_count`, and values are shaped by the authorising account's interests, with no documented enum for `category`. Only worth a user-token flow if the counts prove necessary. |
