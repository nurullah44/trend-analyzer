...
| **X API** — List timelines `GET /2/lists/{id}/tweets` | Posts from a list the owner curated: text, author, `created_at`, `public_metrics` (likes, replies, reposts, quotes) | **$0.005 per post read** — 2 lists × 8 posts daily ≈ $2.40/month; 100 posts/day ≈ $15/month | The deliberate alternative to algorithmic trends: the owner controls the source by curating list membership. Supported by the same credentials as the app-only calls. Posts are RT-heavy, and an RT's own metrics read as zero while the original's repost count survives, so RTs must be deduped. X's `context_annotations` labels exist but proved noisy and partial ("Babies" on a memorial post), so topic Labels stay ours. |
